...
contains
  ...
  module procedure leg_xxdx_with_tfm_kit
    ...
    select type(kit)
      ...
      type is (tfm_kit_2d)
        if (findloc(kit%m, mval, dim=1) .eq. 0) then
         stop 'leg_xxdx_with_tfm_kit: m out of the transformation kit'
        endif
      type is (tfm_kit_3d)
        if (findloc(kit%m, mval, dim=1) .eq. 0) then
    ...
    do i = 1, n_input
      do j = 1, n_output
        genm_xxdx(i,j) = genm_xxdx(i,j) * exp(kit%lognorm(j, am+1) - kit%lognorm(i, am+1))
      enddo
    enddo
  ...
  module procedure leg_del2h_with_tfm_kit
    ...
    do i = 1, n_input
      do j = 1, n_output
        genm_del2h(i,j) = genm_del2h(i,j) * exp(kit%lognorm(j, am+1) - kit%lognorm(i, am+1))
      enddo
    enddo
  ...
  module procedure leg_del2_with_tfm_kit
    ...
    do i = 1, n_input
      do j = 1, n_output
        genm_del2(i,j) = genm_del2(i,j) * exp(kit%lognorm(j, am+1) - kit%lognorm(i, am+1))
      enddo
    enddo
