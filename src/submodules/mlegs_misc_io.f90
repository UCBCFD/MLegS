...
contains
  ...
  module procedure read_input
    ...
    if (ie .ne. 0) error stop 'read_input: could not open input file'
    ...
    if (verify(trim(flddir), 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_./-') .ne. 0) then
      error stop 'read_input: invalid field output directory'
    endif
    call execute_command_line('mkdir -p -- "'//trim(flddir)//'"', wait=.true., exitstat=exitstat, cmdstat=cmdstat)
    if ((cmdstat .ne. 0) .or. (exitstat .ne. 0)) error stop 'read_input: could not create field output directory'
    ...
    if (verify(trim(logdir), 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_./-') .ne. 0) then
      error stop 'read_input: invalid log output directory'
    endif
    call execute_command_line('mkdir -p -- "'//trim(logdir)//'"', wait=.true., exitstat=exitstat, cmdstat=cmdstat)
    if ((cmdstat .ne. 0) .or. (exitstat .ne. 0)) error stop 'read_input: could not create log output directory'
    ...
    if (ie .eq. 0) then
      read(fu,*,iostat=ie) dum
      if (ie .eq. 0) then
        read(fu,*,iostat=ie) is_svv, svv_cutoff, svv_target, svv_strength, svv_relax
        if (ie .ne. 0) error stop 'read_input: invalid SVV controls'
      endif
    endif

    if ((nr .le. 0) .or. (mod(nr,2) .ne. 0) .or. (np .le. 0) .or. (nz .le. 0)) then
      error stop 'read_input: nr must be positive and even; np and nz must be positive'
    endif
    if ((nrchop .le. 0) .or. (nrchop .gt. nr) .or. (npchop .le. 0) .or. (npchop .gt. np/2+1) .or. &
        (nzchop .le. 0) .or. (nzchop .gt. nz/2+1)) error stop 'read_input: invalid chopping sizes'
    if ((ell .le. 0.D0) .or. (zlen .le. 0.D0)) error stop 'read_input: ell and zlen must be positive'
    if ((dt .le. 0.D0) .or. (totaltime .lt. 0.D0) .or. (totaln .lt. 0)) error stop 'read_input: invalid time stepping values'
    if ((visc .lt. 0.D0) .or. (hypervisc .lt. 0.D0)) error stop 'read_input: viscosities must be non-negative'
    if ((hyperpow .ne. 0) .and. (hyperpow .ne. 4) .and. (hyperpow .ne. 6) .and. (hyperpow .ne. 8)) then
      error stop 'read_input: hyperpow must be zero, 4, 6, or 8'
    endif
    if ((hyperpow .ne. 0) .and. (hypervisc .le. 0.D0)) error stop 'read_input: hypervisc must be positive when enabled'
    if ((fldsavintvl .le. 0) .or. (logsavintvl .le. 0)) error stop 'read_input: output intervals must be positive'
    if ((svv_cutoff .le. 0.D0) .or. (svv_cutoff .ge. 1.D0) .or. (svv_target .le. 0.D0) .or. &
      ...
      error stop 'read_input: invalid SVV controls'
    endif
