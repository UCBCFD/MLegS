...
!=======================================================================
! [USAGE]:
! STUDY THE EFFECT OF LAMBDA - PERTURBATION SIZE
!
...
! PERTURBED SYSTEM
REAL(P8), DIMENSION(10):: LAMB_CUST_ARRAY
...
! PERTURBD SYS:
CALL LINSPACE(0.01,0.1,LAMB_CUST_ARRAY)
DO IS = 1,SIZE(LAMB_CUST_ARRAY)
    LAMB_CUST = LAMB_CUST_ARRAY(IS)
    ...
    DO II = 1,SIZE(SYS_EIG)
        IF (ABS(REAL(SYS_EIG(II))) .GT. 0.05*LAMB_CUST) THEN 
            ...
            WRITE(*,*) REAL(SYS_EIG(II))/LAMB_CUST,',',AIMAG(SYS_EIG(II)), ' , ', &
                EIGRES(SYS_EIG_VEC(1:N_mat1,II),M_1), EIGRES(SYS_EIG_VEC(N_mat1+1:,II),M_2)
        ...
        ENDIF
    ENDDO
    DEALLOCATE(SYS_EIG,SYS_EIG_VEC); SYS_NEW = 0.D0
ENDDO
DEALLOCATE(SYS_ORG,SYS_BAR,SYS_NEW)
! ======================================================================
...
CALL SAVE_PSICHI(EIG_R_mat_1_1(:, JJ),'./perturb/pt_MK_'//ITOA3(M_1)//'_'//K_VAL//'_O.output')
WRITE(K_VAL,'(F06.2)') K_VAL_2_1
! CALL SAVE_PSICHI(VECR_2,'./perturb/pt_MK_'//ITOA3(M_2)//'_'//K_VAL//'_O.output')
CALL SAVE_PSICHI(EIG_R_mat_2_1(:, KK),'./perturb/pt_MK_'//ITOA3(M_2)//'_'//K_VAL//'_O.output')
