...
CALL EIG_MATRIX(M_BAR, K_BAR, M_mat, M_eig, EIG_R_mat, EIG_L_mat, comm_grp=newcomm, print_switch=.false.) ! calculate operator matrix and its corresponding eigenvalues
...
IF (MPI_GLB_RANK.EQ.0) THEN
    ...
    !=======================================================================
    ! DO II = 1,SIZE(M_eig_1_1)
    !     IF (aimag(M_eig_1_1(II)).gt.0.d0) WRITE (*, *) M_eig_1_1(II)
    ! ENDDO
    ! WRITE(*,*) '2:'
    ! DO II = 1,SIZE(M_eig_2_1)
    !     IF (aimag(M_eig_2_1(II)).lt.-1.d0) WRITE (*, *) M_eig_2_1(II)
    ! ENDDO
    ! ======================================================================
...
DO II = 1,SIZE(SYS_EIG)
    IF (ABS(REAL(SYS_EIG(II))) .GT. 0.05*LAMB_CUST) THEN 
        ...
        IF (REAL(SYS_EIG(II)) .GT. 0.D0) CALL PERTURB_SAVE(SYS_EIG_VEC(:,II),M_1, K_VAL_1_1, M_2, K_VAL_2_1, newcomm)
    ENDIF
    
    ! WRITE(*,*) '#',ITOA3(II),': ',SYS_EIG(II) 
    !RTOA6(REAL(SYS_EIG(II))),'+',RTOA6(AIMAG(SYS_EIG(II))),'i'
    ! IF (ABS(AIMAG(SYS_EIG(II))-AIMAG(M_eig_1_0(JJ)-K_VAL_1_1*EIG_BAR/K_BAR)).LT.0.1) WRITE(*,*) '#',ITOA3(II),': ',SYS_EIG(II)
ENDDO
DEALLOCATE(SYS_ORG,SYS_BAR,SYS_NEW,SYS_EIG)
! ======================================================================
...
! ======================================================================
    ...
    IF (MPI_GLB_RANK .EQ. 0) THEN  ! START OF THE SERIAL PART
        IF (PRESENT(switch)) THEN
            ...
            END DO
            close (10)
            write (*, *) 'PERTURB MAT (M=',ITOA3(M_END),', AK=',RTOA6(AK_END),'):  max element = ', RTOA6(maxval(abs(H))),' - LOCATION:',maxloc(abs(H))
            write (*, *) 'PERTURB MAT: maxUR = ', maxval(abs(RUR_0(1:NR)/TFM%R))
            write (*, *) 'PERTURB MAT: maxUP = ', maxval(abs(RUP_0(1:NR)/TFM%R))
            write (*, *) 'PERTURB MAT: maxUZ = ', maxval(abs( UZ_0(1:NR)))
            write (*, *) 'PERTURB MAT: maxOR = ', maxval(abs(ROR_0(1:NR)/TFM%R))
...
SUBROUTINE PERTURB_SAVE(EIG_VEC,M1,AK1,M2,AK2,comm_grp)
! ======================================================================
...
NR_MK_1 = MAX(MIN(NRCHOP,NRCHOP-M1),0)
NR_MK_2 = MAX(MIN(NRCHOP,NRCHOP-M2),0)
...
DEL2CHI1(1:NR_MK_2) = EIG_VEC(2*NR_MK_1+NR_MK_2+1:)
...
CALL CHOPSET(3); CALL PC2VEL_MK(PSI1,CHI1,RUR,RUP,UZ); CALL CHOPSET(-3)
...
CALL CHOPSET(3); CALL PC2VEL_MK(PSI2,CHI2,RUR,RUP,UZ); CALL CHOPSET(-3)
CALL RTRAN_MK(RUR,1); CALL RTRAN_MK(RUP,1); CALL RTRAN_MK(UZ,1)
