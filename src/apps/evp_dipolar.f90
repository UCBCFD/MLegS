...
!=======================================================================
! [USAGE]:
! CALCULATE DIPOLAR(M=1) CORRECTION DUE TO CURVATURE
! OBTAIN RESONANT PAIRS AND CALCULATE EIGVAL CORRECTOIN
!
! [UPDATES]:
...
IMPLICIT NONE
INTEGER     :: II, JJ, KK
INTEGER     :: M_BAR, M_1, M_2, tol_ind(2)
...
COMPLEX(P8), DIMENSION(:), ALLOCATABLE:: M_eig_1_0, M_eig_1_1, M_eig_2_0, M_eig_2_1
...
IF (MPI_GLB_RANK .EQ. 0) THEN
    ...
    PSI_BAR = STRAIN_DIPOLAR(newcomm)
    CHI_BAR = 0.D0

    CALL LEGINIT(newcomm,M_BAR) ! NOTE: STRAIN_DIPOLAR changes M
    ...
    RUP_BAR = RUP_BAR + 0.5*TFM%R*(1-EXP(-TFM%R**2)) 
     OZ_BAR =  OZ_BAR !+ (2*TFM%R**2*EXP(-TFM%R**2)+1-EXP(-TFM%R**2))/(2*TFM%R) ! PROBLEMATIC
...
! M_1 = 1; K_VAL_1_0 = 4.0;
! M_1 = 1; K_VAL_1_0 = 5.5;
M_1 = 0; K_VAL_1_0 = 10.7; K_shift = 1.D0 ! Initialization
...
DO II = 1, 6
    ...
    IF (MPI_GLB_RANK.EQ.0) THEN
        ...
        IF ((M_BAR.EQ.M_1).AND.(ABS(K_BAR-K_VAL_1_0).LE.0.1)) THEN
            MASK_1 = MASK_1 .AND. (ABS(M_eig_1_0).GT.0.01)
        ENDIF
        
        ! Find the minimum distance
        tol_diff = MINVAL(ABS(AIMAG(MESHGRID(M_eig_1_0, M_eig_2_0))),MESHGRID(MASK_1,MASK_2))

        ! Reach the desired tolerance
        IF ((tol_diff.LT.tol).OR.(ABS(K_shift).LE.1.0E-6)) THEN

            tol_ind = MINLOC(ABS(AIMAG(MESHGRID(M_eig_1_0, M_eig_2_0))),MESHGRID(MASK_1,MASK_2))
            JJ = tol_ind(1); KK = tol_ind(2)

            WRITE(*,*) 'Desired tol reached w/ ', ITOA3(II-1),' steps (tol_diff = ', tol_diff,' & k_shift = ',abs(k_shift),')'
            WRITE(*,201) M_BAR, K_BAR, 0, 0
    ...
    IF (MPI_GLB_RANK.EQ.0) THEN
        ...
        DO JJ = 1,N_mat1
            ...
            ELSEIF ((M_BAR.EQ.M_1).AND.(ABS(K_BAR-K_VAL_1_0).LE.0.1)) THEN   
                IF (ABS(M_eig_1_0(JJ)).LE.0.01) K_tune(JJ) = 100.D0
            ELSE
                EIG_LMR_1 = 0.D0
                EIG_LMR_1 = &
                ...
                DO KK = 1,N_mat2
                    IF (.NOT.((EIGRES(EIG_R_mat_2_0(:,KK),M_2)))) THEN
                        K_tune_JJ(KK) = 100.D0
                    ELSE
                        K_tune_JJ(KK) = K_delta*AIMAG(M_eig_2_0(KK) - M_eig_1_0(JJ)) / ( &
                                    & EIG_LMR_1 - &
...
IF (MPI_GLB_RANK .EQ. 0) THEN
    ...
    tol_ind = MINLOC(ABS(AIMAG(MESHGRID(M_eig_1_1, M_eig_2_1))),MESHGRID(EIGRES(EIG_R_mat_1_1,M_1),EIGRES(EIG_R_mat_2_1,M_2)))
    JJ = tol_ind(1); KK = tol_ind(2)
...
IF (MPI_GLB_RANK .EQ. 0) THEN
    ...
    DEALLOCATE(M_eig_1_0, M_eig_1_1, M_eig_2_0, M_eig_2_1)
    ...
    IF (ALLOCATED(K_tune)) THEN
        ...
        DEALLOCATE(M_mat_diff_1, M_mat_diff_2)
    ENDIF
    WRITE (*, *) ''
...
! ======================================================================
    ...
    ! PSI
    STRAIN_DIPOLAR = VTHETA*CUMTRAPZ(R,FUNC)

    ! DAMP OUT OUTSIDE REGION
    STRAIN_DIPOLAR = 0.5*E*STRAIN_DIPOLAR*(1-TANH((R-RC)/dR))*0.5

    DEALLOCATE(FUNC,VTHETA,R)

    ! TEST
    ! WRITE(*,'(F20.13)') REAL(STRAIN_DIPOLAR),AIMAG(STRAIN_DIPOLAR)
    ! CALL MPI_ABORT(MPI_COMM_WORLD,1,IERR)
