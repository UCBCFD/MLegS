program tp_project
!> This is a tutorial program to demonstrate how to project a vector field to its solenoidal portion, in terms of toroidal and poloidal scalars. 
!> A vector field, in a component-wise form, is described as: (vr, vp, vz) = ( 0, (1-exp(-r^2))/r, exp(-r^2)/q ), where q = 1
!> vr, vp and vz are calculated in this tutorial program's sequel one (vecfld_reconstruction), which must be run in advance.
...
!> Initialize the global transformation kit
  call tfm%init()
  if ((nr .ne. 200) .or. (np .ne. 128) .or. (nz .ne. 8)) stop 'tp_project: invalid input params.'
...
!!!............ Declare a scalar field and load the component-wise information
...
!> Load 3 vector components
  call mload(trim(adjustl(FLDDIR)) // 'vr' // trim(adjustl(ntoa(ni,'(i0.6)'))) // '_' // &
             vr%space, vr, is_binary = .false., is_global = .true.)
  call mload(trim(adjustl(FLDDIR)) // 'vp' // trim(adjustl(ntoa(ni,'(i0.6)'))) // '_' // &
             vp%space, vp, is_binary = .false., is_global = .true.)
  call mload(trim(adjustl(FLDDIR)) // 'vz' // trim(adjustl(ntoa(ni,'(i0.6)'))) // '_' // &
             vz%space, vz, is_binary = .false., is_global = .true.)

!> Interim time record
  if (rank_glb .eq. 0) then
    write(*,*) 'Vector Field Component Loading'
    write(*,101) toc(); call tic()
    write(*,*) ''
  endif

!!!............ Perform the TP decomposition
!> Run vec2tp()
  call vec2tp(vr, vp, vz, psi, chi, tfm)

!> Save the projected field information, psi and chi
  call msave(psi, trim(adjustl(FLDDIR)) // 'pjpsi' // trim(adjustl(ntoa(ni,'(i0.6)'))) // '_' // &
             psi%space, is_binary = .false., is_global = .true.)
  call msave(chi, trim(adjustl(FLDDIR)) // 'pjchi' // trim(adjustl(ntoa(ni,'(i0.6)'))) // '_' // &
             chi%space, is_binary = .false., is_global = .true.)

!> Interim time record
  if (rank_glb .eq. 0) then
    write(*,*) 'Toroidal-Poloidal Decomposition (Projection)'
    write(*,101) toc(); call tic()
    write(*,*) ''
  endif
