...
REAL(P8), allocatable, dimension(:,:):: ENE_o
...
call allocate(dpsi)
call allocate(dchi)
ALLOCATE(ENE_o(NTCHOP,NXCHOPDIM))
...
do it=1,iii
    ...
    if(monitorData%start.eq.tim%n) then 
        ...
        chi_o = chi
        time0 = tim%t
        ENE_o = ENERGY_SPEC_MODIFIED(psi_o,chi_o)
    end if
...
!     call WRITE_MODE_DATA_TO_FILE(psi,chi)
    ...
    !> clean modes
    call RENORMALIZE(psi,chi,psio,chio,psi_o,chi_o,ENE_o)
...
! ======================================================================
    ...
    call WRITE_ENERGY_DATA_TO_FILE(PSI0,CHIO)
