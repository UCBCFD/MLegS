...
!=======================================================================
      ...
      IF(POSTPROCESS%JOB.EQ.0) THEN
        IF (POSTPROCESS%FINISH .EQ. 1) THEN
          ...
          ELSEIF (POSTPROCESS%SLICEINT.EQ.888) THEN
            POSTPROCESS%SLICEINT = 999
            OMEGAZFILE =  'omegaZ_3D_'//NUM//'.dat'
            CALL MSAVE_SLICES_IN_RTHETA_PLANE(OZ, TRIM(ADJUSTL(FILES%SAVEDIR))//OMEGAZFILE,&
                                            POSTPROCESS%SLICEINT)
            OMEGAZFILE =  'omegaP_3D_'//NUM//'.dat'
            CALL MSAVE_SLICES_IN_RTHETA_PLANE(ROP, TRIM(ADJUSTL(FILES%SAVEDIR))//OMEGAZFILE,&
                                            POSTPROCESS%SLICEINT)
            OMEGAZFILE =  'omegaR_3D_'//NUM//'.dat'
            CALL MSAVE_SLICES_IN_RTHETA_PLANE(ROR, TRIM(ADJUSTL(FILES%SAVEDIR))//OMEGAZFILE,&
                                            POSTPROCESS%SLICEINT)
          ELSE 
