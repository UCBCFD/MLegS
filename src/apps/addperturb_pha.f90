...
chi0%LN = 0
IF (MPI_RANK .EQ. 0) THEN
   WRITE(*,*) 'LOAD PERTURB 1 FILE?[T/F]:'
   READ(*,*) save_q
ENDIF
CALL MPI_BCAST(save_q,1,MPI_LOGICAL,0,MPI_COMM_WORLD,IERR)
IF (save_q) THEN
   call mload(TRIM(ADJUSTL(FILES%SAVEDIR))//"per_perturb_1_psi.dat",psi0)
   call mload(TRIM(ADJUSTL(FILES%SAVEDIR))//"per_perturb_1_chi.dat",chi0)
ENDIF
