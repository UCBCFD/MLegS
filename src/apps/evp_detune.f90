...
COMPLEX(P8) :: EIG_BAR, EIG_PRIME_R, ALPHA, BETA, ALPHA_L, BETA_L 
COMPLEX(P8) :: EIG_1_0, EIG_2_0, EIG_1_1, EIG_2_1
...
! CALL EIG_MATRIX(M_BAR, K_BAR, M_mat, M_eig, EIG_R_mat, EIG_L_mat, comm_grp=newcomm, print_switch=.true.) 
...
DO II = 1,6 ! Maximum number of steps allowed
    ...
    IF (MPI_GLB_RANK.EQ.0) THEN
        ...
        IF ((tol_diff.LT.tol).OR.(ABS(K_shift).LE.1.0E-4)) THEN
            ...
            EIG_1_1 = M_eig_1_0(JJ); EIG_2_1 = M_eig_2_0(KK)
...
! Detuning
! ======================================================================
! Initialization - Tuned case
K_VAL_1_1 = K_VAL_1_0
! Detuning loop
DO II = 1,100

    ! IF (II.EQ.2) K_VAL_1_1 = real(NINT(K_VAL_1_0*50))/50
    ...
    RANK0: IF (MPI_GLB_RANK .EQ. 0) THEN
        ...
        ! JJ = tol_ind(1); KK = tol_ind(2)
        JJ = MINLOC(ABS(M_eig_1_1-EIG_1_1),1)
        KK = MINLOC(ABS(M_eig_2_1-EIG_2_1),1)

        EIG_1_1 = M_eig_1_1(JJ); EIG_2_1 = M_eig_2_1(KK)
        ! IF (II.EQ.2) THEN ! save the rounded triad
        IF (II.EQ.1) THEN ! save the rounded triad
            EIG_1_0 = EIG_1_1; EIG_2_0 = EIG_2_1
        ENDIF
        ...
        IF (.NOT.(ALLOCATED(RUR_BARC))) THEN
        ...
        ! WRITE(*,*) " <L1NR2> ", DOT_PRODUCT(VECL_1, VECR_B2), "<L2NR1>", DOT_PRODUCT(VECL_2, VECR_B1)
        ...
        ! DEALLOCATE
        !=======================================================================
    1   DEALLOCATE (RUR_1, RUP_1, UZ_1, ROR_1, ROP_1, OZ_1)
        DEALLOCATE (RUR_2, RUP_2, UZ_2, ROR_2, ROP_2, OZ_2)
        DEALLOCATE (VECR_B1, VECR_B2, VECR_1, VECR_2, VECL_1, VECL_2)
        DEALLOCATE (EIG_R_mat_1_1, EIG_L_mat_1_1, EIG_R_mat_2_1, EIG_L_mat_2_1)

        ! SAVE DETUNE RESULT
        ! ======================================================================
    298 FORMAT(3(I3,',',F08.4,',',I3,',',2(G20.12,',')),3(G20.12,','),G20.12)

        ! IF (REAL(EIG_PRIME_R).GT.0.01) THEN
        LAMB_CUST = 0.20
        WRITE(*,*) REAL(REAL((M_eig_1_1(JJ) + EIG_BAR - M_eig_2_1(KK))**2/4 + LAMB_CUST**2 * EIG_PRIME_R)**0.5)/LAMB_CUST

        open(334, FILE='evp_detune.dat', STATUS='unknown', ACTION='WRITE', ACCESS='APPEND', IOSTAT=IS)
        WRITE(334,298) M_BAR, K_BAR, EIG_BAR_IND, REAL(EIG_BAR), AIMAG(EIG_BAR), &
            M_1, K_VAL_1_1, JJ, REAL(M_eig_1_1(JJ)), AIMAG(M_eig_1_1(JJ)), &
            M_2, K_VAL_2_1, kk, REAL(M_eig_2_1(KK)), AIMAG(M_eig_2_1(KK)), &
            ABS(REAL(EIG_PRIME_R**0.5)), AIMAG(EIG_PRIME_R**0.5), &
            REAL((M_eig_1_1(JJ) + EIG_BAR - M_eig_2_1(KK))**2/4), &
            REAL(EIG_PRIME_R)
        close(334)

        ! if lambda = 0.2 case gives imaginary sigma correction
        ! 1. switch to the other side of the perfectly tuned triad
        ! 2. abort the program
        IF (REAL((M_eig_1_1(JJ) + EIG_BAR - M_eig_2_1(KK))**2/4 + 0.2**2 * EIG_PRIME_R).LE.0.D0) THEN
            IF (SGN_FLIP .EQ. -1) CALL MPI_ABORT(MPI_COMM_WORLD,336,IERR)
            SGN_FLIP = -SGN_FLIP
            EIG_1_1 = EIG_1_0; EIG_2_1 = EIG_2_0
        ENDIF        
    ...
    ! UPDATE K_VAL_1_1
    CALL MPI_ALLREDUCE( MPI_IN_PLACE, SGN_FLIP, 1, MPI_INTEGER, MPI_MIN, MPI_COMM_WORLD, IERR)
    ! IF ((K_VAL_1_1 .LT. K_VAL_1_0).AND.(SGN_FLIP.LT.0)) K_VAL_1_1 = real(NINT(K_VAL_1_0*50))/50
    IF ((K_VAL_1_1 .LT. K_VAL_1_0).AND.(SGN_FLIP.LT.0)) K_VAL_1_1 = K_VAL_1_0
    K_VAL_1_1 = K_VAL_1_1 - 0.025 * SGN_FLIP

    IF (ABS(K_VAL_1_1-K_VAL_1_0).GT.ABS(K_VAL_1_0)/2) GOTO 10
...
IF (ALLOCATED(VECR_BAR)) DEALLOCATE(VECR_BAR,VECL_BAR)
IF (ALLOCATED(M_mat_1_1)) DEALLOCATE(M_mat_1_1,M_mat_2_1)
